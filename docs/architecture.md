...
| `/news/1?source=rbc&q=москва` | источник и поиск одновременно |
...
Mos.ru и РБК отдают разное количество записей; API по умолчанию берёт максимум 50 записей каждого источника. RSS РБК содержит анонсы и enclosure-изображения, поэтому он заменил Lenta.ru с её часто пустым `<description>`. Приложение не генерирует выдуманный текст и не скрапит страницы: поиск использует title и description из RSS.
